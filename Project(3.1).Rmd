...
```{r}
ggplot(Total_NY_Wifi, aes(x=Percent_Rural_Pop, y=Percent_Cable)) + 
  geom_point(aes(size=estimate, color=Percent_White))
```


```{r}
CNY<-as.data.frame(Total_NY_Wifi) # convert from tibble to data frame
row.names(CNY)<-CNY$County
CNY<-CNY[,2:8]
CNY_mat <- data.matrix(CNY)

par(mar = rep(2, 4))

heatmap.2(CNY_mat, Rowv=NA, Colv=NA, scale="column",
